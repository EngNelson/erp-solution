import {
  ...
  import { CancelStoragePointService } from './cancel-storage-point.service';
  ...
  export class CancelStoragePointController {
    ...
    @ApiResponse({
      status: 201,
      type: CancelStoragePointInput,
    })
    async cancelStoragePoint(
      @Body() body: CancelStoragePointInput,
      @Query() params: ISOLandDto,
      @UserConnected() user: UserCon,
      @Req() request: any,
    ): Promise<CancelStoragePointInput> {
      if (!user) {
      ...
      return await this._cancelStoragePointService.cancelStoragePoint(body, user, accessToken);
    }
