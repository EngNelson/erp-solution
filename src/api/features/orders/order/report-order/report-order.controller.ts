import {
  Body,
  Controller,
  NotFoundException,
  Patch,
  Query,
  UseGuards,
} from '@nestjs/common';
import { ApiResponse, ApiTags } from '@nestjs/swagger';
import { ReportOrderService } from './report-order.service';
import { OrderItemOutput } from 'src/domain/dto/orders';
import { JwtAuthGuard, PermissionsGuard, RolesGuard } from 'src/api/guards';
import {
  Abilities,
  AgentRoles,
  ApiRessources,
  ISOLandDto,
  ISOLang,
  RequirePrivileges,
  Ressource,
  Roles,
  UserCon,
  UserConnected,
} from '@glosuite/shared';
import { ReportOrderInput } from './dto';

@ApiTags('orders')
@Controller('orders')
export class ReportOrderController {
  constructor(private readonly _reportOrderService: ReportOrderService) {}

  @UseGuards(JwtAuthGuard, RolesGuard, PermissionsGuard)
  @Roles(
    AgentRoles.SUPER_ADMIN,
    AgentRoles.FLEET_SUPERVISOR,
    AgentRoles.LOGISTIC_MANAGER,
    AgentRoles.PUS_MANAGER,
    AgentRoles.PUS_COORDINATOR,
    AgentRoles.PUS_AGENT,
  )
  @Ressource(ApiRessources.ORDERS)
  @RequirePrivileges(Abilities.MANAGE, Abilities.UPDATE)
  @Patch('report-delivery')
  @ApiResponse({
    status: 201,
    type: OrderItemOutput,
  })
  async reportOrder(
    @Body() body: ReportOrderInput,
    @Query() params: ISOLandDto,
    @UserConnected() user: UserCon,
  ): Promise<OrderItemOutput> {
    if (!user) {
      throw new NotFoundException(`User not found`);
    }

    body.lang = params.lang
      ? params.lang
      : user.preferedLang
      ? user.preferedLang
      : ISOLang.FR;

    return await this._reportOrderService.reportOrder(body, user);
  }
}
